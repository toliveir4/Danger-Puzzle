...
bool addPrimeiraLinha(Board *b, int row, int col, pair<int, int> p) {
    for (auto &match: pairs[p]) {
        if (!match->used) {
            if (match->num[3] == p.second && match->num[2] == p.first) match->Rotate90();

            if (match->num[2] == p.second && match->num[1] == p.first) match->Rotate180();

            if (match->num[1] == p.second && match->num[0] == p.first) match->Rotate270();
            ...
            if (col == 1) { // se estiver no inicio da linha volta para o final da linha anterior
                row--;
                col = b->columns;
            } else
                col--;
...
bool addPrimeiraColuna(Board *b, int row, int col, pair<int, int> p) {
    for (auto &match: pairs[p]) {
        if (!match->used) {
            ...
            if (match->num[0] == p.second && match->num[3] == p.first) match->Rotate90();

            if (match->num[3] == p.second && match->num[2] == p.first) match->Rotate180();

            if (match->num[2] == p.second && match->num[1] == p.first) match->Rotate270();
...
bool addPeca(Board *b, int row, int col, vector<Piece *> intersection, vector<int> trio) {
    for (auto &match: intersection) {
        if (!match->used) {
            ...
            if (match->num[0] == trio[2] && match->num[3] == trio[1] && match->num[2] == trio[0]) match->Rotate90();

            if (match->num[3] == trio[2] && match->num[2] == trio[1] && match->num[1] == trio[0]) match->Rotate180();

            if (match->num[2] == trio[2] && match->num[1] == trio[1] && match->num[0] == trio[0]) match->Rotate270();
